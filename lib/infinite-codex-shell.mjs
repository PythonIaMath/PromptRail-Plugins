...
export async function installInfiniteCodexShellAlias({ helperPath, previousState = {}, environment = process.env } = {}) {
  ...
  const line = `alias infinite=${shellQuote(`${helperPath} codex -p infinite`)}`;
  const managed = `# PromptRail Infinite profile (managed)\n${line}\n`;
  let original = "";
  let existed = true;
  try {
  ...
  if (
    previousState.shellAliasPath === path
    && previousState.shellAliasContents
    && original.endsWith(previousState.shellAliasContents)
  ) {
    original = original.slice(0, -previousState.shellAliasContents.length);
  }
  if (original.includes("# PromptRail Infinite profile (managed)") && !original.endsWith(managed)) {
