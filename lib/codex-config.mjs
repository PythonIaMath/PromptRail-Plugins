...
export function infiniteBaseUrl(value = process.env.PROMPTRAIL_INFINITE_BASE_URL) {
  ...
  if (!["http:", "https:"].includes(parsed.protocol) || parsed.username || parsed.password) {
    throw new Error("PROMPTRAIL_INFINITE_BASE_URL must be an absolute HTTP(S) URL without credentials.");
  }
  return baseUrl;
}
...
export function infiniteInstallStatePath() {
  return process.env.PROMPTRAIL_INFINITE_HOME
    ? join(process.env.PROMPTRAIL_INFINITE_HOME, "install-state.json")
    : join(homedir(), ".codex", "promptrail-infinite", "install-state.json");
}
...
export function patchInfiniteCodexConfig(original, baseUrl = infiniteBaseUrl()) {
  if (original.includes(`[model_providers.${INFINITE_PROVIDER_NAME}]`)) {
...
export function unpatchInfiniteCodexConfig(current, original) {
  const originalLines = original.split("\n");
  let lines = current.split("\n");
...
export async function installInfiniteCodexConfig(
  ...
  const installed = patchInfiniteCodexConfig(original, baseUrl);
  await mkdir(dirname(path), { recursive: true, mode: 0o700 });
  await mkdir(dirname(statePath), { recursive: true, mode: 0o700 });
  await writeFile(path, installed, { mode: 0o600 });
  await chmod(path, 0o600);
  await writeFile(
    statePath,
    `${JSON.stringify({ configPath: path, original, baseUrl, installedSha256: sha256(installed) }, null, 2)}\n`,
    { mode: 0o600 },
  );
  await chmod(statePath, 0o600);
  return { path, statePath };
...
export async function uninstallInfiniteCodexConfig(statePath = infiniteInstallStatePath()) {
  ...
  try {
    ...
    if (error?.code === "ENOENT") return state.configPath;
    throw error;
  }
  const restored = sha256(current) === state.installedSha256
    ? state.original
    : unpatchInfiniteCodexConfig(current, state.original);
  await writeFile(state.configPath, restored, { mode: 0o600 });
  await chmod(state.configPath, 0o600);
  return state.configPath;
}
