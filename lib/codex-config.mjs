...
export function buildInfiniteModelCatalog(records = []) {
  ...
  const directRecords = records.filter((record) => record?.id !== "promptrail/infinite");
  const seen = new Set();
  const directModels = directRecords.map((record, index) => {
    if (!record || typeof record !== "object" || Array.isArray(record)) {
      throw new Error("Infinite model record is invalid.");
    }
    const slug = safeCatalogString(record.id, "id", 300);
    if (
      !/^promptrail\/direct-[a-z0-9._:+-]+$/.test(slug)
      || slug.split("/").length !== 2
      || record.object !== "model"
      || record.routing_mode !== "direct-free-v1"
      || seen.has(slug)
    ) {
      throw new Error(`Infinite direct model id is invalid: ${slug}.`);
    }
    seen.add(slug);
    ...
    if (
      ...
      throw new Error(`Infinite direct model ${slug} is not coding-harness compatible.`);
    }
  ...
  return {
    models: [structuredClone(INFINITE_MODEL_TEMPLATE), ...directModels],
  };
}
...
function hasExpectedInfiniteProviderFields(lines, expectedBaseUrl, requireDiagnosticsHeader) {
  ...
  if (!diagnostics) return !requireDiagnosticsHeader;
  return /^\s*http_headers\s*=\s*\{\s*"X-PromptRail-Diagnostics"\s*=\s*"executed-model"\s*\}\s*(?:#.*)?$/.test(diagnostics);
}

function assertInfiniteManagedConfigUnchanged(current, state, modelCatalogPath) {
  const lines = current.split("\n");
  if (
    !hasExpectedManagedTopLevelString(lines, "model", "promptrail/infinite")
    || !hasExpectedManagedTopLevelString(lines, "model_provider", INFINITE_PROVIDER_NAME)
    || !hasExpectedManagedTopLevelString(lines, "model_catalog_json", modelCatalogPath)
...
export async function upgradeInstalledInfiniteCodexConfig({
  ...
  const installed = patchInfiniteCodexConfig(preservedOriginal, baseUrl, modelCatalogPath);
  await mkdir(dirname(path), { recursive: true, mode: 0o700 });
  await mkdir(dirname(statePath), { recursive: true, mode: 0o700 });
