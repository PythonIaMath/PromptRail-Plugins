...
function infiniteProviderBlock(baseUrl, helperPath) {
  return `
...
[model_providers.promptrail-infinite.auth]
...
export function patchInfiniteCodexConfig(
  ...
  helperPath = infiniteApiKeyHelperPath(),
) {
  if (original.includes(`[model_providers.${INFINITE_PROVIDER_NAME}]`)) {
  ...
  return `${lines.join("\n").replace(/\s+$/, "")}${infiniteProviderBlock(baseUrl, helperPath)}`;
}
