...
async function startDetachedProxy(values) {
  ...
  if (child.exitCode !== null) {
    throw new Error(`PromptRail Claude proxy exited during startup with code ${child.exitCode}.`);
  }
  return { manager: "detached", path, label: "promptrail-claude-router" };
}

async function stopDetachedProxy() {
  await stopDetachedProxyProcesses(detachedServicePath(), "promptrail-claude-router");
}
