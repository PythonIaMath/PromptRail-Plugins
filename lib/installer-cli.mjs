...
import {
  ...
  DEFAULT_PROMPTRAIL_ACCOUNT_URL,
} from "./infinite-browser-auth.mjs";
...
Environment:
  PROMPTRAIL_ACCOUNT_URL       Browser login origin (default ${DEFAULT_PROMPTRAIL_ACCOUNT_URL})
  PROMPTRAIL_ACCESS_TOKEN       Plugins token or non-interactive Infinite override
  PROMPTRAIL_ROUTER_TOKEN       Alias for PROMPTRAIL_ACCESS_TOKEN
  PROMPTRAIL_API_KEY            Non-interactive Infinite API-key override
...
export async function runCli({
  ...
  authenticateInfinite = authenticateInfiniteCli,
} = {}) {
  ...
  if (["install", "switch"].includes(parsed.command)) {
    ...
    if (!token) {
      token = parsed.mode === "infinite"
        ? await authenticateInfinite({
          accountBaseUrl: processEnv.PROMPTRAIL_ACCOUNT_URL,
          detectedHarnesses: parsed.target === "both" ? ["codex", "claude"] : [parsed.target],
          output: stdout,
        })
        : await readSecret({ input: stdin, output: stdout });
    }
