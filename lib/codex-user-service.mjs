...
import { stopDetachedProxyProcesses } from "./detached-proxy-process.mjs";
...
async function startDetachedProxy(values) {
  ...
  let child;
  try {
    child = spawn(values.nodePath, [values.proxyPath], {
      detached: true,
      env: { ...process.env, PROMPTRAIL_ROUTER_CONFIG: values.configPath },
      stdio: ["ignore", log.fd, log.fd],
    });
    child.unref();
    if (!child.pid) {
      throw new Error("PromptRail could not start a detached Codex proxy process.");
    }
  ...
  if (child.exitCode !== null) {
    throw new Error(`PromptRail Codex proxy exited during startup with code ${child.exitCode}.`);
  }
  return { manager: "detached", path, label: "promptrail-codex-router" };
}

async function stopDetachedProxy() {
  await stopDetachedProxyProcesses(detachedServicePath(), "promptrail-codex-router");
}
